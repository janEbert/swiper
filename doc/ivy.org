...
Ivy is installed as part of the =ivy= package, which is available from
...
Ivy is split into three packages: =ivy=, =swiper= and =counsel=; you
can simply install =counsel= which will bring in the other two as
dependencies. See the code below for adding MELPA to the list of
package archives:
